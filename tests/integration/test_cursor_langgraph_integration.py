...
def create_mock_substrate_service():
    """Create a mock MemorySubstrateService with governance context support."""
    mock_service = Mock(spec=MemorySubstrateService)
    mock_service.write_packet = AsyncMock()
    mock_service.semantic_search = AsyncMock()
    mock_service.search_packets_by_thread = AsyncMock()
    return mock_service
...
class TestDecisionWrittenToPacketStoreV2:
    ...
    async def test_decision_written_to_packet_store_v2(self):
        ...
        mock_service = create_mock_substrate_service()
        mock_service.write_packet = AsyncMock(
...
class TestScopeEnforcementCursorCannotReadLPrivate:
    ...
    async def test_scope_enforcement_cursor_cannot_read_l_private(self):
        ...
        with patch(
            ...
            # Should return empty list (mock returns None for semantic_search)
