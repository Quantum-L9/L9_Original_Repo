...
def test_factory_function_naming_informational():
    ...
    factory_patterns = {"create_": [], "build_": [], "make_": [], "get_": []}
    ...
    for file_path in python_files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                for prefix in factory_patterns:
...
class MissingSingletonVisitor(ast.NodeVisitor):
    ...
    def visit_ClassDef(self, node):
        ...
        has_register_singleton = (
            "@register_singleton" in self.content
            and node.name.lower() in self.content.lower()
        )
                ...
                if isinstance(item, ast.Call):
                    if isinstance(item.func, ast.Attribute):
                        if item.func.attr == "sleep":
                            has_sleep = True
                        if (
                            "status" in item.func.attr.lower()
                            or "state" in item.func.attr.lower()
                        ):
                            has_status_check = True

            if has_sleep and has_status_check:
                self.suggestions.append(
                    {"line": node.lineno, "pattern": "Polling loop detected"}
                )
...
def test_observer_pattern_opportunity():
    ...
    for file_path in python_files:
        ...
        if visitor.suggestions:
            suggestions.append(
                {"file": str(file_path), "suggestions": visitor.suggestions}
            )
...
def test_adr_enforcement_summary():
    ...
    for file_path in python_files:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                if any(
                    node.name.startswith(p)
                    for p in ["create_", "build_", "make_", "get_"]
                ):
    ...
    print(
        f"🟠 Direct agent communication: {counts['direct_agent_communication']} (ADR-0060)"
    )
    print(
        f"🟢 Complex subsystem access:   {counts['complex_subsystem_access']} (ADR-0061)"
    )
    print(
        f"🟢 Factory functions:          {counts['factory_functions']} (informational)"
    )
    print(f"🟡 Missing singleton getter:   {counts['missing_singleton']} (ADR-0004)")
    print(
        f"🟢 Polling patterns:           {counts['polling_patterns']} (informational)"
    )
    print("=" * 60)
