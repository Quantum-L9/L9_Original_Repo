...
sys.path.insert(
    0, str(Path(__file__).parent.parent.parent.parent / "scripts" / "pr_review")
)
...
def test_gemini_auto_editor_imports():
    ...
    try:
        ...
        raise AssertionError(f"Failed to import gemini_auto_editor: {e}") from e
...
def test_perplexity_reviewer_imports():
    ...
    try:
        ...
        raise AssertionError(f"Failed to import perplexity_reviewer: {e}") from e
