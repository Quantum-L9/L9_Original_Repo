...
async def handle_tool_call(
    ...
    requested_scope = (
        tool.arguments.get("scope", "developer")
        if tool.name in {"save_memory", "save_memory_with_confidence"}
        else "developer"
    )
    ...
    context_scope = (
        requested_scope if requested_scope in base_allowed_scopes else base_allowed_scopes[0]
    )
    try:
        governance_ctx = build_governance_context(
            ...
            allowed_scopes=base_allowed_scopes,
            creator=creator,
            source=source,
    ...
    async with governance_context(governance_ctx):
        ...
        try:
            if tool.name == "save_memory":
                ...
                if requested_scope not in base_allowed_scopes:
                    raise ValueError("Scope not authorized for caller.")
                ...
                effective_scopes = [s for s in requested_scopes if s in base_allowed_scopes]
            ...
            await execute(
                ...
                project_id,  # project_id
                json.dumps(tool.arguments, default=str),  # input_data
                json.dumps(
            ...
            await execute(
                ...
                caller_id,
                project_id,
                json.dumps(tool.arguments, default=str),
                json.dumps({"error": error}, default=str),
                duration_ms,
                error,
            )
