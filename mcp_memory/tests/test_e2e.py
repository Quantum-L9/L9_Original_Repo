...
async def test_e2e_save_memory(mock_caller_c):
    ...
    with patch("src.routes.memory_unified.execute") as mock_execute, \
         patch("src.routes.memory_unified.embed_text", new_callable=AsyncMock) as mock_embed, \
...
async def test_e2e_audit_logging_tool_call_logged(mock_caller_c):
    ...
    with patch("src.routes.memory_unified.get_memory_stats", new_callable=AsyncMock) as mock_stats, \
