    ...
    def resolve_finding(
        ...
        if resolved.symbol_type == "dataclass_field":
            try:
                ...
                serial_reason = self.check_serialization_usage(
                    simple_symbol, file_content
                )
                if serial_reason:
                    resolved.is_false_positive = True
                    resolved.false_positive_reason = (
                        f"Serialization via {serial_reason}"
                    )
                    return resolved
            except Exception:
                pass
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="L9 Dead Code Audit - Phase 2: Resolution"
    )
    parser.add_argument(
        "--input",
        type=str,
