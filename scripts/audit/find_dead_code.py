...
def find_unwired_routers(repo_root: Path) -> list[DeadCodeFinding]:
    ...
    main_files = ["main.py", "api/main.py", "api/__init__.py", "app.py", "api/server.py"]
    ...
    for rel_path, routers in router_files.items():
        for line_num, var_name in routers:
            ...
            if var_name not in mounted_routers and f"{var_name}_router" not in mounted_routers:
                findings.append(DeadCodeFinding(
                    file=rel_path,
                    line=line_num,
                    symbol=var_name,
...
def find_unwired_services(repo_root: Path) -> list[DeadCodeFinding]:
    ...
    service_classes: dict[str, tuple[str, int]] = {}  # class_name -> (file, line)
    
    class_pattern = re.compile(r"^class\s+(\w+(?:Service|Executor|Pipeline|Handler|Manager))\s*[:\(]")
    ...
    for class_name, (def_file, def_line) in service_classes.items():
        ...
        inst_patterns = [
            ...
            re.compile(rf"app\.state\.\w+\s*=\s*{class_name}"),  # FastAPI lifespan wiring
        ...
        if not instantiation_found:
            findings.append(DeadCodeFinding(
                ...
                confidence=0.70,  # Lower confidence - may use DI or dynamic loading
                source="wiring_scan",
...
def find_unwired_pydantic_models(repo_root: Path) -> list[DeadCodeFinding]:
    ...
    model_pattern = re.compile(r"^class\s+(\w+(?:Request|Response|Model|Schema|Payload|Input|Output))\s*\(\s*(?:BaseModel|Base\w*Model)")
    ...
    for model_name, (def_file, def_line) in list(pydantic_models.items()):
        ...
        if not usage_found:
            ...
            confidence = 0.60 if "Input" in model_name or "Output" in model_name else 0.80
...
def find_unwired_dependencies(repo_root: Path) -> list[DeadCodeFinding]:
    ...
    dep_func_pattern = re.compile(r"^(?:async\s+)?def\s+(get_\w+|require_\w+|verify_\w+)\s*\(")
    ...
    for func_name, (def_file, def_line) in dep_functions.items():
        if func_name not in used_deps:
            # Lower confidence for dependencies - they're often scaffolding
            findings.append(DeadCodeFinding(
                file=def_file,
                line=def_line,
                symbol=func_name,
                symbol_type="unwired_dependency",
                confidence=0.55,  # Lower confidence: dependency scaffolding
                source="wiring_scan",
                message=f"Dependency '{func_name}' defined but never used with Depends()",
                context="LEGITIMATE: Scaffolding for future route integration",
            ))
...
def find_unwired_orchestrators(repo_root: Path) -> list[DeadCodeFinding]:
    ...
    orch_pattern = re.compile(r"^class\s+(\w+(?:Orchestrator|Interface|Controller))\s*[:\(]")
        ...
        for filepath in python_files:
            ...
            try:
                ...
                for i, line in enumerate(content.split("\n"), 1):
                    ...
                    if re.search(rf"\b{class_name}\s*\(", line) or \
                       re.search(rf":\s*{class_name}\b", line):
                        instantiation_found = True
                        break
            except Exception:
                continue
...
def find_unwired_event_handlers(repo_root: Path) -> list[DeadCodeFinding]:
    ...
    handlers: dict[str, tuple[str, int, str, str]] = {}  # func_name -> (file, line, event_type, content)
                ...
                for pattern, event_type in handler_patterns:
                    ...
                    if match:
                        ...
                        if i > 1:
                            ...
                            if prev_line.startswith("@") and (".get" in prev_line or ".post" in prev_line or ".route" in prev_line):
    ...
    event_pattern = re.compile(r'on_event\s*\(\s*["\'](?:startup|shutdown)["\']\s*\)\s*\(\s*(\w+)')
    decorator_pattern = re.compile(r'@\w+\.on_event\s*\(\s*["\'](?:startup|shutdown)["\']\s*\)')
...
def generate_sarif_output(result: AuditResult, tool_name: str = "l9-dead-code-audit") -> dict:
    """
    ...
    for finding in result.findings:
        ...
        if rule_id not in seen_rules:
            ...
            rules.append({
                ...
                "properties": {
                    "category": "wiring" if finding.source == "wiring_scan" else "dead_code"
                }
            })
...
def print_findings_by_confidence_tier(findings: list[DeadCodeFinding]) -> None:
    ...
    if high:
        ...
        for f in high[:10]:
            print(f"  [{f.confidence:.0%}] {f.symbol} ({f.symbol_type}) @ {f.file}:{f.line}")
        if len(high) > 10:
...
"""
    ...
    l9_types = ["unwired_kernel", "unwired_tool", "unwired_agent", "unwired_orchestrator"]
