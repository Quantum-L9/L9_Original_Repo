...
"""
L9 Runtime Import Validation Audit (Python 3.12)
=================================================
...
Version: 1.1.0 (Python 3.12 compatible)
"""
...
class RuntimeImportValidator:
    ...
    SKIP_DIRS = {
        ...
        'l9_private', '_archived', 'analysis_reports'
    }
    ...
    # Modules that require external services (skip gracefully)
    EXTERNAL_DEPS = {
        'sqlalchemy', 'asyncpg', 'redis', 'neo4j', 'langgraph',
        'langchain', 'langsmith', 'tiktoken', 'anthropic'
    }
    ...
    def is_external_dep_error(self, error_msg: str) -> bool:
        """Check if error is due to missing external dependency."""
        for dep in self.EXTERNAL_DEPS:
            if dep in error_msg.lower():
                return True
        return False
    ...
    def validate_module_import(self, path: Path) -> ValidationResult:
        ...
        try:
            ...
            if self.is_external_dep_error(str(e)):
                result.status = "SKIP"
                result.error_type = "MissingExternalDep"
                result.error_message = str(e)
                result.warnings.append(f"External dependency not installed: {e}")
            else:
                result.status = "FAIL"
    ...
    def run_audit(self, max_files: int | None = None, categories: list[str] | None = None) -> AuditReport:
        """Run the complete audit."""
        print(f"Starting Runtime Import Validation Audit")
        print(f"Python version: {self.report.python_version}")
...
def main():
    ...
    parser.add_argument("--categories", type=str, nargs="+", help="Filter by path categories (e.g., api agents core)")
    args = parser.parse_args()
    
    validator = RuntimeImportValidator(Path(args.root))
    report = validator.run_audit(max_files=args.max_files, categories=args.categories)
