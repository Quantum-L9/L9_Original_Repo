...
def main() -> int:
    ...
    if violations_summary:
        ...
        for filepath, violations in sorted(violations_summary.items()):
            rel_path = Path(filepath).relative_to(repo_root) if filepath.startswith(str(repo_root)) else filepath
            for violation in violations:
                print(f"  {rel_path}: {violation}")
        print()
        print(f"Total: {sum(len(v) for v in violations_summary.values())} issues in {len(violations_summary)} files")
        return 1
    else:
