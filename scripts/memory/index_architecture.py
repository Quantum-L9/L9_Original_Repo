                ...
                if match:
                    ...
                    for file_match in file_matches:
                        ...
                        for keyword_pattern in decision_keywords:
                            keyword_match = re.search(keyword_pattern, context, re.IGNORECASE)
                            if keyword_match:
...
async def main(dry_run: bool = False, verbose: bool = False):
    ...
    if verbose:
        ...
        for decision in all_decisions[:5]:
            logger.info(
                f"  {decision['component']}: {decision['rationale'][:50]}..."
            )
    ...
    try:
        ...
        result = await index_architecture_decisions(all_decisions, service, dry_run=dry_run)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Index architectural decisions to memory graph")
