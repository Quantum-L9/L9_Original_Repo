    ...
    async with httpx.AsyncClient(verify=False, timeout=60.0) as client:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if results_list:
                    print(f"✅ Sample scores: {results['semantic_memory']['sample_scores']}")
            else:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    results["neo4j_knowledge"] = {"status": f"❌ Query failed: {data.get('error')}"}
            else:
            ...
            if r.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    for agent in agents[:5]:
                        print(f"   - {agent.get('agent_id')}: {agent.get('designation', 'N/A')}")
                else:
                    results["agent_state"] = {"status": f"❌ Query failed: {data.get('error')}"}
            else:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if data.get("success") and data.get("data"):
                    ...
                    print(f"✅ Total events: {results['event_timeline']['total_events']}")
                    print(f"✅ Event types: {results['event_timeline']['event_types']}")
                else:
                    results["event_timeline"] = {"status": f"❌ Query failed: {data.get('error')}"}
            else:
            ...
            if r.status_code == 200:
                ...
                if data.get("success"):
                    ...
                    results["repo_structure"] = {
                        "status": "✅ ACCESSIBLE",
                        "node_types": {n.get("type"): n.get("count") for n in repo_nodes},
                        "total_nodes": sum(n.get("count", 0) for n in repo_nodes),
                    }
                    print(f"✅ Total repo nodes: {results['repo_structure']['total_nodes']}")
                    for node in repo_nodes[:5]:
                        print(f"   - {node.get('type')}: {node.get('count'):,}")
                else:
                    results["repo_structure"] = {"status": f"❌ Query failed: {data.get('error')}"}
            else:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                results["world_model"] = {"status": "⚠️ NOT AVAILABLE (endpoint not found)"}
        ...
        accessible = sum(1 for r in results.values() if r.get("status", "").startswith("✅"))
        total = len(results)
        
        for name, result in results.items():
            status = result.get("status", "UNKNOWN")
            print(f"{'✅' if status.startswith('✅') else '❌' if status.startswith('❌') else '⚠️'} {name.upper().replace('_', ' ')}: {status}")
