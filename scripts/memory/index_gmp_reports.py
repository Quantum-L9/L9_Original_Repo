...
def parse_gmp_report(file_path: Path) -> Optional[Dict[str, Any]]:
    ...
    decisions = []
    change_map_match = re.search(r'## TODO → CHANGE MAP(.*?)(?=##|\Z)', content, re.DOTALL)
    if change_map_match:
    ...
    files_modified = []
    files_section = re.search(r'## FILES MODIFIED(.*?)(?=##|\Z)', content, re.DOTALL | re.IGNORECASE)
    if files_section:
        ...
        for decision in report_data.get("decisions", [])[:10]:  # Limit to 10
            ...
            try:
                await substrate_service._repository.insert_knowledge_fact(
                    subject=gmp_id,
                    predicate="decided",
                    object_value={"decision": decision, "title": report_data.get("title")},
                    confidence=0.9,
