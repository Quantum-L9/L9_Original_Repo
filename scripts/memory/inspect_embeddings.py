    ...
    try:
        ...
        try:
            ...
            for row in rows:
                try:
                    ...
                    if isinstance(text, str):
                        ...
                        if "slack" in text.lower() and ("glitch" in text.lower() or "retry" in text.lower()):
                            suspicious_patterns["slack_glitch"] += 1
                        if text.startswith("{") and text.endswith("}"):
                            suspicious_patterns["json_dump"] += 1
                    
                    # Categorize payload type
                    payload_type = payload.get("type") or payload.get("packet_type") or "unknown"
            ...
            return {
                ...
                "agent_counts": {row["agent_id"] or "null": row["count"] for row in agent_counts},
...
async def main(limit: int = 50, agent_id: Optional[str] = None, sample: bool = False):
    ...
    result = await inspect_embeddings(DATABASE_URL, limit=limit, agent_id=agent_id, sample=sample)
    ...
    print("\nEmbeddings by Agent:")
    for agent, count in sorted(result["agent_counts"].items(), key=lambda x: x[1], reverse=True):
        print(f"  {agent or '(null)':20} {count:>8,}")
    
    print(f"\nPayload Types (sample of {result['sample_size']}):")
    for ptype, count in sorted(result["payload_types"].items(), key=lambda x: x[1], reverse=True):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit", type=int, default=50, help="Number of samples to inspect")
    parser.add_argument("--agent-id", type=str, help="Filter by agent_id")
    parser.add_argument("--sample", action="store_true", help="Random sample instead of most recent")
