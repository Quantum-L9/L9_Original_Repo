...
async def audit_packets(limit: int = 10) -> Dict[str, Any]:
    ...
    result = await api_request("POST", "/api/v1/memory/semantic/search", json={
        "query": "memory",
        "top_k": limit,
        "min_score": 0.0,
    })
...
async def audit_neo4j_via_api() -> Dict[str, Any]:
    ...
    node_stats = await api_request("POST", "/api/v1/memory/graph/query", json={
    ...
    rel_stats = await api_request("POST", "/api/v1/memory/graph/query", json={
    ...
    agent_state = await api_request("POST", "/api/v1/memory/graph/query", json={
        "query": """
        MATCH (a:Agent)
    ...
    event_stats = await api_request("POST", "/api/v1/memory/graph/query", json={
        "query": """
        MATCH (e:Event)
    ...
    event_types = await api_request("POST", "/api/v1/memory/graph/query", json={
        "query": """
        MATCH (e:Event)
    ...
    if event_stats.get("success") and "data" in event_stats:
        results["events"] = {
            "stats": event_stats.get("data", [{}])[0] if event_stats.get("data") else {},
            "event_types": event_types.get("data", []) if event_types.get("success") and "data" in event_types else [],
        }
    else:
    ...
    repo_stats = await api_request("POST", "/api/v1/memory/graph/query", json={
        "query": """
        MATCH (n)
...
async def main():
    ...
    try:
        ...
        # Save to file
        output_file = Path(__file__).parent.parent / "reports" / f"graph_audit_vps_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
