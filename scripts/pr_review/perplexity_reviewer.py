...
"""
    ...
    def generate_audit_report(self, audit: Dict) -> str:
        ...
        for finding in audit.get("security_findings", [])[:5]:
            emoji = {"critical": "🔴", "high": "🟠", "medium": "🟡", "low": "🟢"}.get(finding.get("severity", "low"), "⚪")
            md.append(f"{emoji} **{finding.get('severity', 'unknown').upper()}**: {finding.get('issue', 'No details')}")
            md.append(f"  File: `{finding.get('file', 'unknown')}` (Line {finding.get('line', '?')})\n")
        
        md.append("\n### ⚡ Performance Opportunities\n")
        for opp in audit.get("performance_opportunities", [])[:5]:
            md.append(f"- **{opp.get('opportunity', 'Unknown')}** ({opp.get('impact', 'unknown')} impact)")
