...
class GenerationResult:
    ...
    api_response: Optional[PerplexityResponse] = None
...
class FileExtractor:
    ...
    def extract_files(self, response_text: str) -> List[ExtractedFile]:
        ...
        files.extend(self._extract_by_pattern(
            response_text, self.PYTHON_BLOCK, "python"
        ))
        
        # Extract YAML files
        files.extend(self._extract_by_pattern(
            response_text, self.YAML_BLOCK, "yaml"
        ))
        
        # Extract Markdown (for README)
        files.extend(self._extract_by_pattern(
            response_text, self.MARKDOWN_BLOCK, "markdown"
        ))
...
__dora_footer__ = {
    ...
    "audit_trail": True,
    "dependencies": [],
    "tags": ["api", "async", "auth", "caching", "cli", "client", "code-generation", "dataclass", "filesystem", "foundation"],
    "keywords": ["audit", "client", "codegen", "compliance", "extract", "extracted", "extractor", "files"],
