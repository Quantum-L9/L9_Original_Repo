...
```bash
# Copy environment template (repo root)
cp .env.template .env
...
### 3. Start Services (ADR-0089: hierarchical compose)

```bash
# Development (base + dev overlay)
docker compose -f docker-compose.yml -f docker-compose.dev.yml up -d

# Production (base + prod overlay, env file)
docker compose -f docker-compose.yml -f docker-compose.prod.yml --env-file .env up -d
```

**C1:** Use `deploy/c1/deploy.sh`; secrets in `.env.c1` (gitignored). See `deploy/c1/README.md`.
...
### Development Setup (ADR-0089)

**Files:** `docker-compose.yml` (base) + `docker-compose.dev.yml` (overlay) at repo root
...
# View logs
docker compose -f docker-compose.yml -f docker-compose.dev.yml logs -f l9-api
...
### Production Setup (ADR-0089)

**Files:** `docker-compose.yml` (base) + `docker-compose.prod.yml` (overlay) at repo root

**Features:**

- Multi-stage root Dockerfiles (development + production targets)
- Resource limits in prod overlay
- Health checks
- C1: use `deploy/c1/deploy.sh` and `.env.c1` (symlinks recommended)

```bash
# Build production images (root Dockerfiles)
docker compose -f docker-compose.yml -f docker-compose.prod.yml build

# Start with environment file
docker compose -f docker-compose.yml -f docker-compose.prod.yml --env-file .env up -d

# C1: ./deploy/c1/deploy.sh (uses .env.c1)
...
```bash
# Build specific image (root Dockerfile)
docker build -f Dockerfile --target production -t l9-api:4.0.0 .
