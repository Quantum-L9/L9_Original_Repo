...
class TaggedClaim:
    ...
    def to_inline(self) -> str:
        ...
        return tag_claim(self.text, self.confidence, self.status, self.sources, inline=True)
    
    def to_verbose(self) -> str:
        """Return verbose tagged format."""
        return tag_claim(self.text, self.confidence, self.status, self.sources, inline=False)
...
def format_for_transparency(
    ...
    if level == TransparencyLevel.LEVEL_0:
        ...
        lines.append(f"- Overall confidence: {summary['overall_confidence']*100:.0f}%")
