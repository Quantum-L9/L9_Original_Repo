...
**Report Period:** {datetime.now(UTC).strftime("%Y-%m-%d")} (Last 7 days)
...
**Scheduled:** {(datetime.now(UTC) + timedelta(days=1)).strftime("%Y-%m-%d 02:00 UTC")}
...
_Report saved to: `{self.report_dir / f"report_{datetime.now(UTC).strftime('%Y%m%d')}.md"}`_
"""

        # Save report
        report_path = (
            self.report_dir / f"report_{datetime.now(UTC).strftime('%Y%m%d')}.md"
        )
