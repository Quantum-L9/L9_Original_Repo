...
__dora_meta__ = {
    ...
    "module_name": "calibration_dashboard",
    "type": "dataclass",
    "status": "active",
    "integrates_with": {
        "api_endpoints": [],
...
**Report Period:** {datetime.now(timezone.utc).strftime("%Y-%m-%d")} (Last 7 days)
...
**Scheduled:** {(datetime.now(timezone.utc) + timedelta(days=1)).strftime("%Y-%m-%d 02:00 UTC")}
...
_Report saved to: `{self.report_dir / f"report_{datetime.now(timezone.utc).strftime('%Y%m%d')}.md"}`_
"""

        # Save report
        report_path = (
            self.report_dir
            / f"report_{datetime.now(timezone.utc).strftime('%Y%m%d')}.md"
        )
    ...
    def _calculate_metrics(self, decisions: List[Dict]) -> CalibrationMetrics:
        ...
        if high_risk_predictions:
            ...
            precision = (
                tp / len(high_risk_predictions) if high_risk_predictions else 0.0
            )
        else:
            precision = 0.0
            tp = 0
    ...
    def _calculate_ece(self, decisions: List[Dict]) -> float:
        ...
        for bin_decisions in bins:
            ...
            confidence = sum(
                d.get("calibrated_probability", 0.5) for d in bin_decisions
            ) / len(bin_decisions)
    ...
    def _breakdown_by_model(self, decisions: List[Dict]) -> str:
        ...
        for model, model_decisions in by_model.items():
            ...
            avg_confidence = sum(
                d.get("confidence", 0.5) for d in model_decisions
            ) / len(model_decisions)
    ...
    def _analyze_thresholds(self, decisions: List[Dict]) -> str:
        ...
        for threshold_name, threshold_value in thresholds.items():
            above = [
                d
                for d in decisions
                if d.get("calibrated_probability", 0) >= threshold_value
            ]
            ...
            if above:
                ...
                lines.append(
                    f"### {threshold_name.replace('_', ' ').title()} (≥{threshold_value})"
                )
                lines.append(f"- Triggered: {len(above)} times")
                lines.append(f"- Precision: {precision:.2%}")
                lines.append(
                    f"- Status: {'✅ Good' if precision > 0.80 else '⚠️ Needs adjustment'}"
                )
                lines.append("")
    ...
    def _generate_recommendations(
        self, metrics: CalibrationMetrics, decisions: List[Dict]
    ) -> str:
        ...
        if metrics.ece > 0.08:
            recommendations.append(
                "⚠️ **High calibration error** - Run temperature optimization"
            )
        ...
        if metrics.accuracy < 0.85:
            recommendations.append(
                "⚠️ **Low accuracy** - Review evidence weights and model logic"
            )
        ...
        if metrics.recall < 0.75:
            recommendations.append(
                "⚠️ **Low recall** - Missing risky situations, lower thresholds"
            )
...
__dora_footer__ = {
    ...
    "business_value": "Provides calibration dashboard components including CalibrationMetrics, CalibrationDashboard",
