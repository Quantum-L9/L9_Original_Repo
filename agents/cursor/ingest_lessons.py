...
def write_lesson_to_mcp(lesson: dict) -> dict:
    ...
    result = mcp_call_tool(
        "save_memory",
        {
            ...
            "tags": tags,
            "importance": 1.0
            if lesson["severity"] in ("ULTRA_CRITICAL", "CRITICAL")
            else 0.8,
            "metadata": {
                "agent": "cursor",
                "domain": "governance",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Ingest repeated-mistakes.md lessons into MCP memory"
    )
    parser.add_argument(
        "--live",
        action="store_true",
    ...
    if not lessons_path.exists():
        print(f"ERROR: File not found: {lessons_path}")  # noqa: ADR-0019
        sys.exit(1)
    ...
    if args.json:
        ...
        print(json.dumps(output, indent=2))  # noqa: ADR-0019
    else:
        mode_label = "🔴 LIVE" if is_live else "🟢 DRY-RUN"
        print(f"\n{'=' * 60}")  # noqa: ADR-0019
        print(f"  L9 Lesson Ingestion — {mode_label}")  # noqa: ADR-0019
        print(f"  Source: {lessons_path}")  # noqa: ADR-0019
        print(f"  Total lessons parsed: {len(lessons)}")  # noqa: ADR-0019
        print(f"{'=' * 60}\n")  # noqa: ADR-0019
        ...
        for i, lesson in enumerate(lessons, 1):
            ...
            print(f"  [{i:02d}] {lesson['lesson_id']}")  # noqa: ADR-0019
            print(f"       Title:    {lesson['title'][:60]}")  # noqa: ADR-0019
            print(f"       Severity: {lesson['severity']}")  # noqa: ADR-0019
            print(f"       Quality:  {lesson['quality']}")  # noqa: ADR-0019
            print("       Kind:     lesson")  # noqa: ADR-0019
            print("       Scope:    cursor")  # noqa: ADR-0019
            print(f"       Tags:     {tags}")  # noqa: ADR-0019

            if is_live:
                result = write_lesson_to_mcp(lesson)
                status = "✅" if not result.get("error") else "❌"
                print(f"       Write:    {status} {result}")  # noqa: ADR-0019
            else:
                print("       Write:    [SKIPPED — dry-run]")  # noqa: ADR-0019
        ...
        print(f"    Curated:        {curated}")  # noqa: ADR-0019
        print(f"    Auto-generated: {auto}")  # noqa: ADR-0019
        print(f"    Ultra-critical: {ultra}")  # noqa: ADR-0019
        print(f"    Critical:       {critical}")  # noqa: ADR-0019

        if not is_live:
            print(f"\n  To write for real: python3 {__file__} --live")  # noqa: ADR-0019
        print()  # noqa: ADR-0019
