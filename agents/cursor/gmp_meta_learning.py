...
class GMPExecutionResult(BaseModel):
    ...
    task_type: str = Field(
        ..., description="Type of task (refactor, schema, feature, etc.)"
    )
    ...
    final_confidence: float = Field(
        ..., ge=0, le=100, description="Audit confidence score"
    )
...
class AutonomyGraduationMetrics(BaseModel):
    ...
    current_level: AutonomyLevel = Field(
        AutonomyLevel.L2, description="Current autonomy level"
    )
    perfect_executions_l2: int = Field(
        0, ge=0, description="Consecutive perfect L2 executions"
    )
    consistency_score_l3: float = Field(
        0, ge=0, le=1, description="L3 consistency metric"
    )
...
class GMPMetaLearningEngine:
        ...
        try:
            async with self.async_session() as session:
                ...
                stats = {
                    ...
                    "avg_execution_time": sum(e.execution_minutes for e in exec_list)
                    / len(exec_list),
                    "avg_confidence": sum(e.final_confidence for e in exec_list)
                    / len(exec_list),
                    "error_rate": sum(1 for e in exec_list if e.error_count > 0)
                    / len(exec_list),
                    "pass_rate": sum(1 for e in exec_list if e.audit_result == "PASS")
                    / len(exec_list),
                }
                ...
                for task_type in set(e.task_type for e in exec_list):
                    ...
                    by_task_type[task_type] = {
                        ...
                        "avg_time": sum(e.execution_minutes for e in task_execs)
                        / len(task_execs),
                        "avg_confidence": sum(e.final_confidence for e in task_execs)
                        / len(task_execs),
                    }
    ...
    async def generate_heuristics(self) -> List[LearnedHeuristic]:
        ...
        try:
            async with self.async_session() as session:
                ...
                if high_conf_execs:
                    error_rate_high = sum(
                        1 for e in high_conf_execs if e.error_count > 0
                    ) / len(high_conf_execs)
                    if error_rate_high < 0.1:
                        heuristics.append(
                            LearnedHeuristic(
                                ...
                                confidence=0.92,
                                supporting_gmp_ids=[
                                    e.gmp_id for e in high_conf_execs[:5]
                                ],
    ...
    async def update_autonomy_metrics(
        self, execution: GMPExecutionResult
        ...
        try:
            async with self.async_session() as session:
                ...
                is_perfect = (
                    ...
                    and execution.final_confidence >= 95
                    and execution.audit_result == "PASS"
                )
                ...
                recent_stmt = (
                    select(GMPExecutionHistoryDB)
                    .order_by(GMPExecutionHistoryDB.created_at.desc())
                    .limit(10)
                )
                ...
                if len(recent_execs) >= 5:
                    ...
                    variance = sum((c - avg_conf) ** 2 for c in confidences) / len(
                        confidences
                    )
...
class AutonomyController:
    ...
    async def assert_capability(self, feature: str) -> bool:
        ...
        is_enabled = (
            level_order[current_level.value] >= level_order[required_level.value]
        )
...
async def main():
    ...
    # Note: URL is auto-converted to postgresql+asyncpg://
    engine = GMPMetaLearningEngine(
        database_url="postgresql://user:password@localhost/l9"
    )
