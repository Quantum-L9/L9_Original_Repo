...
**L9 Integration:** This template aligns with L9 GMP v1.0. See `G-CMP-L9-GMP-Alignment.md` for phase mapping. For L9-specific paths and commands, use `g-cmp-l9-special.md`.
...
## Phase Mapping to L9 GMP v1.0

When using G-CMP in L9 context, map phases as follows:

| G-CMP Phase | L9 GMP Phase | Notes |
|-------------|--------------|-------|
| Phase -1: PLAN | Phase 0: TODO PLAN LOCK | Create TODO plan with TODO INDEX HASH |
| Phase 0: VERIFY | Phase 1: BASELINE CONFIRMATION | Verify all TODO targets exist |
| Phase 1: CODE | Phase 2: IMPLEMENTATION | Execute locked TODO plan |
| Phase 2: GUARD | Phase 3: ENFORCEMENT | Add guards, validation, fail-fast |
| Phase 3: SAFETY | Phase 3: ENFORCEMENT | Additional safety (combined with Phase 2) |
| Phase 4: TEST | Phase 4: VALIDATION | Unit + integration + critical-path tests |
| Phase 5: AUDIT | Phase 5: RECURSIVE VERIFICATION + Phase 6: FINAL AUDIT | Split into recursive check + final audit |

## L9 GMP Report Generation

When completing G-CMP work in L9:
...
**Reference:** See `G-CMP-L9-GMP-Alignment.md` for detailed mapping and integration checklist.

## L9-Specific Considerations

- **Tier Classification:** Classify files as KERNEL/RUNTIME/INFRA/UX tier
- **Protected Files:** Core files require explicit approval (see L9 GMP protected_files list)
- **Memory Integration:** Use `/mem` protocol to READ context and WRITE insights
- **Report Format:** Follow L9 GMP report structure (see `.cursor-commands/commands/gmp.md`)
...
**L9 GMP Integration:** v1.0 (see G-CMP-L9-GMP-Alignment.md)
