...
def enricher():
    ...
    world_model.query_causal_factors = AsyncMock(return_value=[
        CausalFactor("cf_1", "influence", 0.8, "positive")
    ])
    
    memory = AsyncMock()
    memory.query_episodic_memory = AsyncMock(return_value=[
        EpisodicEvent("e1", "2026-01-02", "test", {})
    ])
