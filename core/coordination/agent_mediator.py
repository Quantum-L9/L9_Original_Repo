...
Usage:
    from core.coordination.agent_mediator import AgentMediator, get_mediator
...
class AgentMediator:
    ...
    async def send_message(
        ...
        if to_agent in self.agents and self.agent_status.get(to_agent, False):
            ...
            logger.warning(
                f"Agent {to_agent} offline, message queued",
                message_id=msg.id
            )
    ...
    async def acknowledge_message(self, message_id: str, agent_id: str) -> None:
        ...
        if message_id in self.delivery_status:
            ...
            logger.debug(
                f"Message acknowledged by {agent_id}",
    ...
    async def _deliver_message(self, agent_id: str, message: Message) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Message delivered to {agent_id}",
    ...
    async def _deliver_queued_messages(self, agent_id: str) -> None:
        ...
        logger.info(
            f"Delivering {len(messages)} queued messages to {agent_id}"
        )
    ...
    def get_delivery_status(self, message_id: str) -> Optional[MessageDeliveryStatus]:
        """
...
# Convenience function to get singleton mediator instance
