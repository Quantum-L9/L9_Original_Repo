...
class DeriveType(str, Enum):
    """How the information was originally obtained (provenance origin)."""

    DIRECT = "direct"  # First-hand observation/capture
    INFERRED = "inferred"  # Derived through reasoning from other facts
    SYNTHESIZED = "synthesized"  # Generated by combining multiple sources


# Valid derive_type values for validation
VALID_DERIVE_TYPES: set[str] = {dt.value for dt in DeriveType}
...
class PacketWriteResult(BaseModel):
    ...
    enrichment_facts_count: int = Field(
        default=0, description="Number of knowledge facts extracted and persisted"
    )

    # Resilience tracking fields (v2.1.0 - GMP-67 unified pipeline)
