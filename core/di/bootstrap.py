...
def bootstrap_di_container(container: Any) -> Dict[str, int]:
    ...
    try:
        ...
        container.bind_singleton(
            type(get_db_client()),
            lambda: get_db_client()
        )
        registered_count += 1
        logger.debug("di_bootstrap.registered", service="PostgresClient")
    except Exception as e:
        logger.warning("di_bootstrap.optional_service_skipped", service="PostgresClient", reason=str(e))
        optional_skipped += 1
    ...
    try:
        ...
        container.bind_singleton(
            type(get_neo4j_client()),
            lambda: get_neo4j_client()
        )
        registered_count += 1
        logger.debug("di_bootstrap.registered", service="Neo4jClient")
    except Exception as e:
        logger.warning("di_bootstrap.optional_service_skipped", service="Neo4jClient", reason=str(e))
        optional_skipped += 1
    ...
    try:
        ...
        container.bind_singleton(
            type(get_redis_client()),
            lambda: get_redis_client()
        )
        registered_count += 1
        logger.debug("di_bootstrap.registered", service="RedisClient")
    except Exception as e:
        logger.warning("di_bootstrap.optional_service_skipped", service="RedisClient", reason=str(e))
        optional_skipped += 1
    ...
    try:
        from memory.substrate_service import MemorySubstrateService, create_substrate_service
        container.bind_singleton(
            MemorySubstrateService,
            lambda: create_substrate_service()
        )
        registered_count += 1
        logger.debug("di_bootstrap.registered", service="MemorySubstrateService")
    except Exception as e:
        logger.warning("di_bootstrap.optional_service_skipped", service="MemorySubstrateService", reason=str(e))
        optional_skipped += 1
    ...
    try:
        ...
        logger.debug("di_bootstrap.optional_service_skipped", service="AgentPersistenceService", reason=str(e))
        optional_skipped += 1
    ...
    try:
        ...
        container.bind_singleton(
            ...
            lambda: create_executor_tool_registry()
        )
        registered_count += 1
        logger.debug("di_bootstrap.registered", service="ExecutorToolRegistry")
    except Exception as e:
        logger.warning("di_bootstrap.optional_service_skipped", service="ExecutorToolRegistry", reason=str(e))
        optional_skipped += 1
    ...
    try:
        ...
        container.bind_singleton(
            AgentRegistry,
            lambda: create_agent_registry()
        )
        registered_count += 1
        logger.debug("di_bootstrap.registered", service="AgentRegistry")
    except Exception as e:
        logger.warning("di_bootstrap.optional_service_skipped", service="AgentRegistry", reason=str(e))
        optional_skipped += 1
    ...
    try:
        ...
        logger.debug("di_bootstrap.optional_service_skipped", service="ApprovalManager", reason=str(e))
        optional_skipped += 1
    ...
    try:
        ...
        container.bind_singleton(
            AIOSRuntime,
            lambda: create_aios_runtime()
        )
        registered_count += 1
        logger.debug("di_bootstrap.registered", service="AIOSRuntime")
    except Exception as e:
        logger.warning("di_bootstrap.optional_service_skipped", service="AIOSRuntime", reason=str(e))
        optional_skipped += 1
    ...
    try:
        ...
        container.bind_singleton(
            KernelProtocol,
            lambda: load_kernel_protocol()
        )
        registered_count += 1
        logger.debug("di_bootstrap.registered", service="KernelProtocol")
    except Exception as e:
        logger.warning("di_bootstrap.optional_service_skipped", service="KernelProtocol", reason=str(e))
        optional_skipped += 1
