...
class HybridInferenceKernel:
    ...
    def _assess_file_edit_risk(self, context: Dict) -> GovernanceDecision:
        ...
        return GovernanceDecision(
            decision_id=f"GOV-{int(datetime.now(timezone.utc).timestamp())}",
...
__dora_footer__ = {
    ...
    "audit_trail": True,
    "dependencies": [],
    "tags": ["api", "auth", "current-work", "dataclass", "debugging", "filesystem", "logging", "metrics", "migration", "monitoring"],
    "keywords": ["action", "decision", "evaluate", "governance", "hybrid", "inference", "kernel", "predicate"],
