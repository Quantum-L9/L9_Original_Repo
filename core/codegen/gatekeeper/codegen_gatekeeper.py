...
class BlindSpot(BaseModel):
    ...
    category: str = Field(..., description="Category of blind spot (security, performance, edge_case, etc.)")
    ...
    confidence: float = Field(..., ge=0, le=100, description="Confidence that this is a real gap")
...
class NormalizedSpec(BaseModel):
    ...
    normalized_format: str = Field(default="module_spec_v2.6", description="Target spec format")
    spec_data: dict[str, Any] = Field(..., description="Normalized spec in Module-Spec v2.6 format")
...
class CodeGenGatekeeperAgent(BaseAgent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.perplexity_model = "llama-3.1-sonar-large-128k-online"  # Sonar model with web search
    ...
    async def process_contract(
        ...
        validation_result = await self._validate_contract(parsed_contract, contract_type)
        
        # Step 3: Detect blind spots
        blind_spots = await self._detect_blind_spots(
            spec=parsed_contract,
            contract_type=contract_type
        )
    ...
    async def _validate_contract(
        ...
        required_fields = {
            ContractType.AGENT_YAML: [
                ...
                "communicationstack", "reasoningengine"
            ],
            ContractType.MODULE_BLOCK: [
                "metadata", "runtime_wiring", "external_surface", "packet_contract"
            ],
            ContractType.SYMCODE: [
                "symbols", "equations"
            ],
            ContractType.CONCEPT: [
                "CONCEPT_NAME", "ARCHITECTURE", "DATA_FLOW"
            ]
        }
    ...
    async def _detect_blind_spots(
        ...
        if "observability" not in spec and "logging" not in spec and "metrics" not in spec:
            blind_spots.append(BlindSpot(
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            for blind_spot in blind_spots:
                try:
                    ...
                    response = await client.post(
                        ...
                        json={
                            ...
                            "messages": [
                                ...
                                {
                                    "role": "user",
                                    "content": blind_spot.research_query
                                }
                            ],
                            "max_tokens": 500,
                    ...
                    self.logger.error(f"Research failed for blind spot: {e}", exc_info=True)
    ...
    async def _normalize_to_module_spec(
        ...
        if contract_type == ContractType.MODULE_BLOCK:
            ...
            module_spec["metadata"] = {
                "module_id": contract.get("system", {}).get("name", "unknown").lower().replace(" ", "_"),
            ...
            module_spec["metadata"] = {
                "module_id": contract.get("CONCEPT_NAME", "unknown").lower().replace(" ", "_"),
    ...
    async def generate_code(
        ...
        git_manager = GitSafetyManager(repo_root=Path(os.getenv("L9_REPO_ROOT", "/home/ubuntu/L9")))
