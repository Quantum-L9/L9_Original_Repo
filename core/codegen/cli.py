...
from .gatekeeper.codegen_gatekeeper import CodeGenGatekeeperAgent, ContractType
...
@cli.command()
@click.option("--input", "-i", required=True, type=click.Path(exists=True), help="Input spec file")
@click.option("--type", "-t", required=True, type=click.Choice(["agent_yaml", "module_block", "symcode", "concept", "partial"]), help="Contract type")
@click.option("--output", "-o", required=True, type=click.Path(), help="Output directory")
@click.option("--research/--no-research", default=True, help="Enable Perplexity research")
@click.option("--min-confidence", default=85.0, type=float, help="Minimum confidence threshold")
def generate(input: str, type: str, output: str, research: bool, min_confidence: float):
    """Generate code from a spec file"""
    
    click.echo(f"🚀 Unified CodeGen System v1.0.0")
    ...
    async def run():
        ...
        if result.success:
            click.echo(f"✅ Code generation successful!")
            click.echo(f"📊 Confidence: {result.confidence * 100:.1f}%")
            click.echo(f"📁 Files generated: {len(result.data.get('files_generated', []))}")
            ...
            click.echo(f"❌ Code generation failed!")
            click.echo(f"📊 Confidence: {result.confidence * 100:.1f}%")
...
@cli.command()
@click.option("--files", "-f", required=True, type=click.Path(exists=True), help="Directory or file to validate")
def validate(files: str):
    ...
    async def run():
        ...
        for gate in report['gates']:
            ...
            click.echo(f"  {status} Gate {gate['gate_id']}: {gate['name']} - {gate['score']:.1f}%")
...
def research(query: str):
    ...
    from .gatekeeper.codegen_gatekeeper import CodeGenGatekeeperAgent, BlindSpot
    ...
    async def run():
        ...
        if findings:
            ...
            click.echo(f"❌ No research results found")
