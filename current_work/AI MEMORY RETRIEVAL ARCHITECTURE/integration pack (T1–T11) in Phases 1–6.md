...
# Generate all artifacts (T1–T11) in Phases 1–6

Phase 0 is complete and approved; proceeding in advisory Execution Mode to generate definitions for all requested artifacts. Below is the **concise, implementation-ready integration pack** you can drop into your repo. All content respects the governance model and GMP constraints from your docs.[^1][^2]
...
Create `docs/cursor/CURSOR-GOD-PROMPT.md`:
...
2. Identify all impacted components (APIs, orchestrators, memory, tests, docs).
...
- Allowed edits: `docs/cursor/*` (for baseline reports only).
...
- No new code changes.
- Only updates to:
  - `docs/cursor/*` (verification notes)
...
Write `docs/cursor/gmp-evidence-{GMP_RUN_ID}.md` with the above structure and content.
...
Create `docs/cursor/CURSOR-RUNBOOK.md`:
...
|-------|-----------------------|-----------|
| 0 | `docs/cursor/*`, `.cursorrules` | Any `*.py`, `*.sql` |
| 1 | `docs/cursor/*` (reports) | Any code files |
...
| 4 | `tests/**`, `docs/**` (if in TODO PLAN) | Core runtime code |
| 5 | `docs/cursor/*` | Any code |
| 6 | `docs/**` | Any code |
...
Create `docs/cursor/governance-reference.md`:
